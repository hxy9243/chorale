...
Measures are written and inclusive (measure 0 for pickup measures, 1-based for standard complete measures). Mid-measure repeat fragments share the same measure number. A single-measure selection sets both fields to the same value and highlights all fragments of the selected measure. Playback, chat, score focus, and annotation spans share this anchor; ranged playback starts or seeks from `startMeasure`. Existing optional beat, voice, ABC-offset, and playback hints remain available.
