...
export const AbcEditor: React.FC<AbcEditorProps> = ({
  ...
  const [measureProgressOffset, setMeasureProgressOffset] = useState<number | null>(null);
  ...
  useEffect(() => {
    if (view !== "measures" || !presentation) {
      setMeasureProgressOffset(null);
      return;
    }
    ...
    const updateProgressOffset = () => {
      ...
      setMeasureProgressOffset(Math.max(0, timelineBounds.bottom - cardBounds.top + 8));
    };
    const frame = window.requestAnimationFrame(updateProgressOffset);
    window.addEventListener("resize", updateProgressOffset);
    const observer = typeof ResizeObserver === "undefined"
      ? null
      : new ResizeObserver(updateProgressOffset);
    observer?.observe(card);
    observer?.observe(timeline);
    return () => {
      window.cancelAnimationFrame(frame);
      window.removeEventListener("resize", updateProgressOffset);
      observer?.disconnect();
    };
  }, [presentation, view]);
  ...
  return (
    ...
    >
      <div className="abc-editor-chrome">
        ...
        {validationMessage && (
          <div
            className={`editor-banner ${validationState === "invalid" ? "error" : "info"}`}
            role="status"
          >
            {validationMessage}
          </div>
        ...
        {draft?.error && (
          <div className="editor-banner error abc-draft-error" role="alert">
            ...
            <button type="button" onClick={() => focusCell(draft.cellId)}>
              Return to measure
            </button>
          </div>
        )}
        ...
        >
          {view === "raw" ? (
            ...
            <div className="abc-measure-view">
                <div
                  className="abc-header-source"
                  aria-label="ABC header fields"
                >
                {presentation.headers.map((header) => {
                    const isEditingHeader =
                      headerDraft?.range.start === header.range.start;
                  return (
                    <div
                  ...
                  return (
                    <section
                      ...
                      data-timeline-measure={measure}
                        style={{
                          minWidth: `max(18rem, calc(${maxMeasureChars + 8}ch + 5.5rem))`,
                        }}
                      key={measure}
                    >
                      <button
      ...
      {visible && view === "measures" && presentation && (
        <div
          className="abc-horizontal-scrollbar"
          style={measureProgressOffset === null ? undefined : { top: measureProgressOffset }}
        >
          <input
            type="range"
